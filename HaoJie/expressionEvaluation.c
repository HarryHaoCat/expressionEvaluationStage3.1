...
/*The function converts infix expression into postfix expression. 
  ...
  If the infix expresssion is legal, convert it into postfix expression and retuen FAILED.  
*/
		...
		*/
		if(!(isdigit(infixExpression[index]) || infixExpression[index] == '+' || infixExpression[index] == ' '  
		    ...
		    if((infixExpression[index] > 0 && infixExpression[index] < 9) 
		    	...
		    	printf("<Error! Illegal escape character \'\\%o\' in the expression>\n", infixExpression[index]);    		 		    			    	    
...
char previous(char* infixExpression, int index){       
	int i= index;               
	while(i > 0 && (infixExpression[--i] == ' ' || infixExpression[i] == '\t'));
...
char next(char* infixExpression, int index){                           
	int i= index;
	while((infixExpression[++i] == ' ' || infixExpression[i] == '\t')&& infixExpression[i] != '\0');
	return infixExpression[i];                
}
